...
observed = lapply(observed,function(x){x-meanobserve})
...
for(i in 1:numFPC){
	...
	if(i==1){
		res_first = first_FPC_2d_image(beta1,observed,timepoints1,timepoints2,basis1,basis2,threshold=1e-4,minit=3)
		result_list[[1]] = res_first
		previous_beta[[1]] = res_first$beta
		pc_list[[1]] = res_first$pc_fit
	}else{
		
		res_second = third_FPC_conditional_2d_image(beta1, pc_index=i, observed,timepoints1,timepoints2,basis1,basis2, betalist =previous_beta , threshold=1e-3,minit=1)
